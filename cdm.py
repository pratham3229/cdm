...
def save_downtime():
    ...
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute('INSERT INTO downtime (start_time, end_time, reason) VALUES (?, ?, ?)',
...
def get_downtime():
    ...
    downtime_list = [{'id': row[0], 'start_time': row[1], 'end_time': row[2], 'reason': row[3]} for row in data]
